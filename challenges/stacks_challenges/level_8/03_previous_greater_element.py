# Level 8c - Previous Greater Element to the Left
# Write previous_greater_elements(values) in O(n).
# Return nearest strictly greater value to the left for each position, else -1.
...
def _run_case_group(group_name, cases):
    for case_index, (case_label, input_value, expected) in enumerate(cases, start=1):
        ...
        actual = previous_greater_elements(copy.deepcopy(input_value))
        _assert_equal(
            actual,
            expected,
            (
