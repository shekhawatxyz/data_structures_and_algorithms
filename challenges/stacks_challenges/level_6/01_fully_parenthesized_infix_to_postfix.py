...
PEDAGOGY_CASES = [
    ...
    ('two nested groups', '( ( 10 - 2 ) / ( 1 + 1 ) )', ['10', '2', '-', '1', '1', '+', '/']),
    ('left-deep addition chain', '( ( ( 1 + 2 ) + 3 ) + 4 )', ['1', '2', '+', '3', '+', '4', '+']),
]
...
INTERACTION_CASES = [
    ('deeply nested mixed operators', '( ( ( 2 * 3 ) + ( 4 / 2 ) ) - 5 )', ['2', '3', '*', '4', '2', '/', '+', '5', '-']),
    ('nested subtraction and multiplication', '( ( 8 - ( 3 + 1 ) ) * ( 2 + 2 ) )', ['8', '3', '1', '+', '-', '2', '2', '+', '*']),
    ('multi-level right nesting', '( 1 + ( 2 * ( 3 + 4 ) ) )', ['1', '2', '3', '4', '+', '*', '+']),
    ('combined divisions', '( ( 20 / 5 ) / ( 2 + 2 ) )', ['20', '5', '/', '2', '2', '+', '/']),
    ('chain with all operators', '( ( ( 9 - 3 ) * 2 ) + ( 8 / 4 ) )', ['9', '3', '-', '2', '*', '8', '4', '/', '+']),
]
...
def _run_case_group(group_name, cases):
    for case_index, (case_label, input_value, expected) in enumerate(cases, start=1):
        if expected is _CASE_EXPECTS_RAISE:
            _assert_raises(
                lambda value=copy.deepcopy(input_value): fully_parenthesized_infix_to_postfix(value),
                (
