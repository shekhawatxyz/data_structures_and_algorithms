...
# Will do this after I have done queues and dequeues
...
def test_01_pedagogy_fifo_order_with_simple_sequence():
    ...
    _assert_equal(
        queue.dequeue(), 1, "First dequeue should return first enqueued value 1."
    )
    _assert_equal(
        queue.dequeue(), 2, "Second dequeue should return second enqueued value 2."
    )
    _assert_equal(
        queue.dequeue(), 3, "Third dequeue should return third enqueued value 3."
    )
...
def test_02_pedagogy_interleaved_enqueue_and_dequeue():
    ...
    _assert_equal(
        queue.dequeue(),
        "a",
        "First dequeue should return a after enqueue(a), enqueue(b).",
    )
    queue.enqueue("c")
    _assert_equal(
        queue.dequeue(),
        "b",
        "Second dequeue should return b, preserving FIFO across transfer.",
    )
...
def test_03_boundaries_dequeue_on_empty_queue_raises():
    queue = StackQueue()
    _assert_raises(
        lambda: queue.dequeue(), "dequeue() on empty queue should raise an error."
    )
...
def test_04_boundaries_off_by_one_after_full_drain_and_refill():
    ...
    for value in [10, 11]:
        queue.enqueue(value)
    _assert_equal(
        queue.dequeue(), 10, "Queue should return first item before draining."
    )
    _assert_equal(
        queue.dequeue(), 11, "Queue should return second item before becoming empty."
    )
    _assert_raises(
        lambda: queue.dequeue(), "Queue should raise immediately after becoming empty."
    )
    ...
    _assert_equal(
        queue.dequeue(),
        12,
        "Queue should work correctly after being emptied and reused.",
    )
...
def test_05_interactions_long_sequence_preserves_fifo_across_transfer_phases():
    ...
    _assert_equal(
        first_half,
        [0, 1, 2, 3, 4],
        "First dequeue phase should return earliest 5 elements in order.",
    )
...
def test_06_interactions_mixed_data_types_remain_ordered():
    ...
    _assert_equal(
        observed, values, "Queue should preserve insertion order for mixed data types."
    )
...
if __name__ == "__main__":
    TEST_CASES = [
        ...
        (
            "pedagogy: interleaved enqueue/dequeue",
            test_02_pedagogy_interleaved_enqueue_and_dequeue,
        ),
        (
            "boundaries: empty dequeue raises",
            test_03_boundaries_dequeue_on_empty_queue_raises,
        ),
        (
            "boundaries: drain + refill off-by-one checks",
            test_04_boundaries_off_by_one_after_full_drain_and_refill,
        ),
        (
            "interactions: long transfer sequence",
            test_05_interactions_long_sequence_preserves_fifo_across_transfer_phases,
        ),
        (
            "interactions: mixed data types",
            test_06_interactions_mixed_data_types_remain_ordered,
        ),
    ]
