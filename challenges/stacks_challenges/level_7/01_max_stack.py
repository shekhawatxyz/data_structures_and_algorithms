...
def test_01_pedagogy_get_max_progression_while_pushing():
    ...
    for index, (value, expected_max) in enumerate(zip(sequence, expected_maxes), start=1):
        stack.push(value)
        _assert_equal(
            stack.get_max(),
            expected_max,
...
def test_02_pedagogy_lifo_and_max_update_while_popping():
    ...
    _assert_equal(stack.get_max(), 7, 'Max should remain 7 after popping non-max value 3.')
    ...
    _assert_equal(stack.get_max(), 2, 'After all 7s are removed, max should update to 2.')
...
def test_04_boundaries_duplicate_max_values_are_tracked_correctly():
    ...
    stack.pop()
    _assert_equal(stack.get_max(), 5, 'Max should stay 5 after popping one duplicate max.')
    stack.pop()
    _assert_equal(stack.get_max(), 5, 'Max should still stay 5 until all max duplicates are removed.')
...
def test_05_interactions_negative_and_positive_mix():
    ...
    _assert_equal(stack.get_max(), 0, 'Max should become 0 when non-negative value is introduced.')
    _assert_equal(stack.pop(), -1, 'Top should pop in LIFO order even with mixed signs.')
...
def test_06_interactions_interleaved_push_pop_peek_get_max():
    ...
    _assert_equal(stack.get_max(), 4, 'max should remain 4 after pushing smaller value 1.')
...
if __name__ == '__main__':
    TEST_CASES = [
        ('pedagogy: max progression on push', test_01_pedagogy_get_max_progression_while_pushing),
        ('pedagogy: lifo + max updates on pop', test_02_pedagogy_lifo_and_max_update_while_popping),
        ('boundaries: empty operations raise', test_03_boundaries_empty_stack_operations_raise),
        ('boundaries: duplicate maxima', test_04_boundaries_duplicate_max_values_are_tracked_correctly),
        ('interactions: mixed sign values', test_05_interactions_negative_and_positive_mix),
        ('interactions: interleaved ops', test_06_interactions_interleaved_push_pop_peek_get_max),
    ]
