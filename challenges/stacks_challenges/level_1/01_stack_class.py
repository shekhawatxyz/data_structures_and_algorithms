...
def test_02_pedagogy_lifo_progression_with_multiple_pushes():
    ...
    _assert_equal(stack.is_empty(), True, 'Stack should be empty after popping all items.')
...
def test_04_boundaries_size_off_by_one_transitions():
    ...
    stack.push('b')
    _assert_equal(stack.size(), expected_sizes[2], 'Size should be 2 after second push.')
    stack.pop()
    _assert_equal(stack.size(), expected_sizes[3], 'Size should return to 1 after one pop.')
    stack.pop()
    _assert_equal(stack.size(), expected_sizes[4], 'Size should return to 0 after popping all items.')
...
def test_05_interactions_interleaved_operations_keep_consistent_state():
    ...
    _assert_equal(stack.is_empty(), True, 'Stack should be empty after full interleaved sequence.')
...
def test_06_interactions_duplicate_values_are_handled_independently():
    ...
    _assert_equal(stack.size(), 3, 'Pushing three duplicate values should still create three stack entries.')
    ...
    _assert_equal(stack.is_empty(), True, 'Stack should be empty after popping all duplicates.')


if __name__ == '__main__':
    TEST_CASES = [
        ('pedagogy: empty -> single push', test_01_pedagogy_starts_empty_then_single_push_behavior),
        ('pedagogy: LIFO progression', test_02_pedagogy_lifo_progression_with_multiple_pushes),
        ('boundaries: empty pop/peek errors', test_03_boundaries_empty_pop_and_peek_raise_errors),
        ('boundaries: size transitions', test_04_boundaries_size_off_by_one_transitions),
        ('interactions: interleaved consistency', test_05_interactions_interleaved_operations_keep_consistent_state),
        ('interactions: duplicate values', test_06_interactions_duplicate_values_are_handled_independently),
    ]
