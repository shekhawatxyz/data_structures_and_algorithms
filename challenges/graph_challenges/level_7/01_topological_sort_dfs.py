# Level 7a - Topological Sort (DFS)
# Return a topological order of a DAG using DFS post-order reversal.

# Complete Exact Problem Statement (from graph-challenges.md):
# ### 7a — `topological_sort_dfs`
...
def topological_sort_dfs(adj):
    raise NotImplementedError('Implement topological_sort_dfs(adj).')
#
#
#
#
...
def test_01_simple_dag():
    # 0->1->2
    adj = _build_adj_list(3, [(0, 1), (1, 2)], directed=True)
    order = topological_sort_dfs(adj)
    _assert_true(_is_valid_topo_order(adj, order),
                 f"Order {order} is not a valid topological sort.")


def test_02_diamond_dag():
    # 0->1, 0->2, 1->3, 2->3
...
def test_03_cycle_raises():
    ...
    _assert_raises(ValueError, lambda: topological_sort_dfs(adj),
                   "Should raise ValueError for cyclic graph.")


def test_04_disconnected_dag():
    # 0->1, 2->3 (two components)
    adj = _build_adj_list(4, [(0, 1), (2, 3)], directed=True)
    order = topological_sort_dfs(adj)
    _assert_true(_is_valid_topo_order(adj, order),
                 f"Order {order} is not a valid topological sort.")
...
def test_05_single_node():
    adj = [[]]
    order = topological_sort_dfs(adj)
    _assert_equal(order, [0], "Single node topo sort.")


if __name__ == "__main__":
    TEST_CASES = [
        ("simple dag", test_01_simple_dag),
        ("diamond dag", test_02_diamond_dag),
        ("cycle raises", test_03_cycle_raises),
        ("disconnected dag", test_04_disconnected_dag),
