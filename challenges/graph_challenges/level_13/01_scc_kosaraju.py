...
def scc_kosaraju(adj):
    raise NotImplementedError('Implement scc_kosaraju(adj).')
#
#
#
#
...
def test_01_pedagogy_single_scc():
    # 0->1->2->0: one SCC
    adj = [[1], [2], [0]]
    sccs = scc_kosaraju(adj)
    _assert_equal(len(sccs), 1, "One SCC for a cycle.")
    _assert_equal(sorted(sccs[0]), [0, 1, 2], "All vertices in one SCC.")


def test_02_dag_each_vertex_own_scc():
    # 0->1->2 (no cycles): each vertex is its own SCC
    adj = [[1], [2], []]
    sccs = scc_kosaraju(adj)
    _assert_equal(len(sccs), 3, "DAG: each vertex is its own SCC.")
    scc_sets = [set(s) for s in sccs]
    _assert_true({0} in scc_sets, "Vertex 0 is its own SCC.")
    _assert_true({1} in scc_sets, "Vertex 1 is its own SCC.")
    _assert_true({2} in scc_sets, "Vertex 2 is its own SCC.")


def test_03_two_sccs():
    # 0->1->0 (SCC {0,1}) and 1->2 (vertex 2 alone)
    adj = [[1], [0, 2], []]
    sccs = scc_kosaraju(adj)
...
def test_04_isolated_vertices():
    ...
    adj = [[], [], []]
    sccs = scc_kosaraju(adj)
    _assert_equal(len(sccs), 3, "Each isolated vertex is its own SCC.")


def test_05_complex_graph():
    # Two cycles connected: 0->1->2->0 and 3->4->3, with 2->3
    adj = [[1], [2], [0, 3], [], [3]]
    # Fix: adj[3] = [4], adj[4] = [3]
    adj = [[1], [2], [0, 3], [4], [3]]
    sccs = scc_kosaraju(adj)
...
if __name__ == "__main__":
    TEST_CASES = [
        ...
        ("DAG", test_02_dag_each_vertex_own_scc),
        ("two SCCs", test_03_two_sccs),
        ("isolated vertices", test_04_isolated_vertices),
        ("complex graph", test_05_complex_graph),
    ]
