...
def test_insert_front_on_empty_list_creates_single_node():
    ...
    _assert_equal(
        _to_list_forward(new_head), [5], "insert_front(None, 5) should create [5]."
    )
...
def test_insert_front_on_non_empty_list_updates_bidirectional_links():
    ...
    _assert_equal(
        _to_list_forward(new_head),
        [1, 2, 3],
        "insert_front should prepend the new value.",
    )
...
def test_insert_front_preserves_existing_tail_chain():
    ...
    _assert_equal(
        _to_list_forward(new_head),
        [3, 4, 5, 6],
        "Existing nodes should remain after new head.",
    )
    _assert_true(
        _verify_bidirectional_links(new_head),
        "All links should stay valid after insert_front.",
    )
...
def test_insert_front_with_duplicate_values():
    ...
    _assert_equal(
        _to_list_forward(new_head),
        [7, 7, 7],
        "insert_front should handle duplicate values.",
    )
...
if __name__ == "__main__":
    TEST_CASES = [
        (
            "empty list insert_front",
            test_insert_front_on_empty_list_creates_single_node,
        ),
        (
            "non-empty insert updates links",
            test_insert_front_on_non_empty_list_updates_bidirectional_links,
        ),
