...
def test_merge_sorted_basic_interleaving():
    ...
    _assert_equal(
        ...
        [1, 2, 3, 4, 5, 6],
        "merge_sorted should merge into sorted order.",
    )
    _assert_true(
        _verify_bidirectional_links(result),
        "All prev/next links should be valid after merge.",
    )
    _assert_equal(
        set(_node_ids(result)),
        expected_ids,
        "merge_sorted should reuse existing nodes only.",
    )
...
def test_merge_sorted_with_one_empty_input():
    ...
    _assert_equal(
        _to_list_forward(result),
        [1, 2, 3],
        "Merging with empty list should return non-empty list.",
    )
...
def test_merge_sorted_with_duplicates():
    ...
    _assert_equal(
        _to_list_forward(result),
        [1, 2, 2, 2, 3],
        "merge_sorted should preserve duplicates correctly.",
    )


def test_merge_sorted_with_both_empty_inputs():
    _assert_true(
        merge_sorted(None, None) is None, "merge_sorted(None, None) should return None."
    )
