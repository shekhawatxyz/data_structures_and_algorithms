...
def test_delete_all_removes_matches_from_all_positions():
    ...
    _assert_equal(
        ...
        [1, 3, 4],
        "delete_all should remove head/middle/tail matches.",
    )
    _assert_true(
        _verify_bidirectional_links(result),
        "All links should be valid after delete_all.",
    )
...
def test_delete_all_when_no_matches_exist():
    ...
    _assert_equal(
        _to_list_forward(result),
        [1, 3, 5],
        "delete_all should leave list unchanged if no matches.",
    )
...
def test_delete_all_when_all_nodes_match_returns_none():
    ...
    _assert_true(
        result is None, "delete_all should return None if all nodes are removed."
    )


def test_delete_all_on_empty_list_returns_none():
    _assert_true(
        delete_all(None, 1) is None, "delete_all(None, value) should return None."
    )
...
if __name__ == "__main__":
    TEST_CASES = [
        (
            ...
            test_delete_all_removes_matches_from_all_positions,
        ),
        ("no matches unchanged", test_delete_all_when_no_matches_exist),
        (
            "all nodes removed returns None",
            test_delete_all_when_all_nodes_match_returns_none,
        ),
