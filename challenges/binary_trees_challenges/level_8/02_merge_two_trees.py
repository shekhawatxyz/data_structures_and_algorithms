...
def merge_two_trees(t1, t2):
    raise NotImplementedError("Implement merge_two_trees(t1, t2).")
...
def test_two_empty_trees_merge_to_none():
    _assert_true(merge_two_trees(None, None) is None, "two empty trees should merge to None.")


def test_one_empty_tree_returns_other_values():
    tree = _make_level_order([1, 2, 3])
    result = merge_two_trees(None, tree)
    _assert_equal(_level_order_values(result), [1, 2, 3], "empty plus tree should keep tree values.")
...
if __name__ == "__main__":
    TEST_CASES = [
        ...
        ("one empty tree returns other values", test_one_empty_tree_returns_other_values),
